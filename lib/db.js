 ...
 *   SCHEMAS
 *
 * Implementation: PostgreSQL via node-postgres (pg), pointed at Supabase's
 *                  own Postgres database (Supabase is just Postgres).
 * Connection:     DATABASE_URL environment variable (Supabase connection
 *                 pooler string, from Project Settings → Database)
 *
 * Notes:
 *   - Uses raw SQL via pg.Pool (lib/db/postgres.js), not the Supabase JS client.
