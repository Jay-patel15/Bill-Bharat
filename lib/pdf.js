 ...
 */
export function generateInvoicePdf({ company, customer, project, invoice, title = "TAX INVOICE", showTax = true, output = "buffer" }) {
  ...
  const projectName = project?.name || invoice?.projectName || "";
  ...
  if (projectName) {
    doc.text(`Site: ${projectName}`, 165, 151);
  }
  ...
  if (projectName) {
    doc.setFont("helvetica", "bold");
    doc.text(`Project / Site: ${projectName}`, borderMargin + 5, by);
    doc.setFont("helvetica", "normal");
    by += 9;
  }
  if (customer.address) {
  ...
  // Consignee Info (Ship To / Site Delivery)
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7.5);
  doc.text("Name of Consignee / Delivery Site (Ship To)", 302, 187);
  doc.setFontSize(8.5);
  
  if (meta.consigneeSameAsBuyer || !meta.consigneeName) {
    const consigneeTitle = projectName ? `${customer.name} (${projectName})` : (customer.name || "");
    doc.text(consigneeTitle, 302, 198);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7);
    let cy = 208;
    if (project?.address) {
      doc.setFont("helvetica", "bold");
      doc.text(`Site Location: ${project.address}`, 302, cy);
      doc.setFont("helvetica", "normal");
      cy += 9;
