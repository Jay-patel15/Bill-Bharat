...
export function generateInvoicePdf({ company, customer, invoice, title = "TAX INVOICE", showTax = true, output = "buffer" }) {
  ...
  // ───────────── Bank details ─────────────
  if (showTax && (company.bankName || company.bankAccountNo || company.bankIfsc || company.bankBranch)) {
    ensureSpace(80);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    ...
    if (company.bankBranch) { doc.text(`Branch: ${company.bankBranch}`, M, ty); ty += 11; }
